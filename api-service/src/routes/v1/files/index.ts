...
const file: FastifyPluginAsync = async (fastify): Promise<void> => {
    ...
    fastify.post<StartProcessingTypes>(
        "/start-processing",
        { schema: startProcessingSchema },
        (request) => startProcessingHandler(fastify, request.body.filePath),
    );
    fastify.delete<DeleteFileTypes>("/:id", { schema: deleteFileSchema }, (request) =>
        deleteFileHandler(fastify, request.params.id),
    );
};
